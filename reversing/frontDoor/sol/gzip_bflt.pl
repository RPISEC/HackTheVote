...
gzip_bflt - convert gzip-compressed bFLT executable files into uncompressed bFLT


=head1 SYNOPSIS

    gzip_bflt zipped_blflt_files...
...
Uses packages C<IO::Compress::Gzip> and C<POSIX>.
...
sub expand_blftZ($) {
    ...
    if(!($unpHdr[9] & 4)) {
        ...
        warn "$bfltZ: Not an uncompressed bFLT file, not gzipped\n";
        return;
    }
    } else {
