...
if ($_SERVER['REMOTE_ADDR'] != '127.0.0.1') {
    die("You (".$_SERVER['REMOTE_ADDR'].") do not have access to this page...");
}
    $s = 1;
if (isset($_GET['s'])) {
    $s = @intval($_GET['s']);
...
if ($s == 3) {
    if (!isset($_GET['txtfirst_name']) || !isset($_GET['txtmiddle_name']) || !isset($_GET['txtname_suffix']) || !isset($_GET['txtLast_name']) || !isset($_GET['txtdob']) || !isset($_GET['txtdl_nmbr']) || !isset($_GET['txtRetypeDL'])) {
        ...
        $q = "INSERT INTO users (firstName,middleName,lastName,suffix,dob,licenceNumber) VALUES ('".$_GET['txtfirst_name']."','".$_GET['txtmiddle_name']."','".$_GET['txtLast_name']."','".$_GET['txtname_suffix']."','".$_GET['txtdob']."','".$_GET['txtdl_nmbr']."');";
        //echo "<!-- ".htmlentities($q)." -->";
        $stmt = $sql_conn->prepare($q);
        if ($stmt === false) {
            die($sql_conn->error);
        }
        $stmt->execute();
