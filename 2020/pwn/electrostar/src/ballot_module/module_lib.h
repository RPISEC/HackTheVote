...
#define FOR_EACH_FUNC(v) \
    ...
    v(4, libc, void*, malloc, size_t size) \
    v(5, libc, void*, free, void* ptr) \
    v(6, libc, void*, memcpy, void *dest, const void *src, size_t n)
...
char* recv_ipc(uint16_t* len) {
    *len = 0;
    read(g_client->ipc_to_child.read, len, 2);
    if (len == 0)
        return NULL;
    // TODO this in uninit
    void * buf = malloc(*len);
    read(g_client->ipc_to_child.read, buf, *len);
    return buf;
}
...
char* ipc_fgets(uint16_t len) {
    send_ipc(2, (char*)&len, 2);
    return recv_ipc(&len);
}
